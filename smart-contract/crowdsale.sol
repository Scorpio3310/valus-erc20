...
contract ValusCrowdsale is owned {
    ...
    function ValusCrowdsale(uint256 _startDay, address _multisig){
        
        startDay = _startDay;             // On deploy set Unix Timestamp
        endDay = startDay + 29 days;      // Days for crowdsale   
        minEthToRaise = 3030 * 10**18;    // Min ethereum for raise            
        maxEthToRaise = 30303 * 10**18;   // Max ethereum for raise               
        multisigAddress = _multisig;      // Multisign Address
    }
  ...
  //  
    ...
    function () payable{
      ...
      if (crowdsaleHasSucessfulyEnded || now > endDay) throw;        // Throw if the Crowdsale has ended     
      if (!crowdsaleHasStarted){                                     // Check if this is the first Crowdsale transaction       
        if (now >= startDay){                                        // Check if the Crowdsale should start        
          crowdsaleHasStarted = true;                                // Set that the Crowdsale has started         
          CrowdsaleStarted(now);                                     // Raise CrowdsaleStarted event     
        } else{
          throw;
        }
      }
      ...
      if (maxEthToRaise > (totalEthRaised + msg.value)){                        // Check if the user sent too much ETH       
        ...
        valusTokenContract.mintTokens(msg.sender, getValusTokenIssuance(now, msg.value));
        if (!softCapReached && totalEthRaised >= minEthToRaise){                // Check if the min treshold has been reached one time        
          CrowdsaleSoftCapReached(now);                                         // Raise CrowdsalesoftCapReached event        
        ...
        valusTokenContract.mintTokens(msg.sender, getValusTokenIssuance(now, maxContribution));
        ...
        CrowdsaleHardCapReached(now);
        hardCapReached = true;
        CrowdsaleEndedSuccessfuly(now, totalEthRaised);      
    ...
    function claimEthIfFailed(){    
      if (now <= endDay || totalEthRaised >= minEthToRaise) throw;            // Check if Crowdsale has failed    
    ...
    function batchReturnEthIfFailed(uint256 _numberOfReturns) onlyOwner{    
      if (now < endDay || totalEthRaised >= minEthToRaise) throw;              // Check if Crowdsale failed  
    ...
    /* Owner sets new address of multisign address */
    ...
    function getValusTokenIssuance(uint256 _now, uint256 _ethSent) constant returns(uint){    // Bonuses
    ...
    function withdrawRemainingBalanceForManualRecovery() onlyOwner{     
      if (this.balance == 0) throw;                                         // Check if there is balance on the contract    
      if (now < endDay) throw;                                              // Check if Crowdsale failed    
